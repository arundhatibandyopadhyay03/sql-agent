...
#                 attachments=[{"file_id": fid, "tools": [{"type": "file_search"}]} for fid in st.session_state.uploaded_file_ids]
#             )
...
# === NEW helper for splitting attachments ===
...
def application():
    ...
    if user_query:
        ...
        # === MODIFIED: split attachments into batches ===
        if not st.session_state.uploaded_file_ids:
            client.beta.threads.messages.create(
                thread_id=st.session_state.thread_id, 
                role="user", 
